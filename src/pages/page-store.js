...
export function initPageStore() {
  ...
  db = getPagesDb();
  db.exec(`
    CREATE TABLE IF NOT EXISTS logical_pages (
...
export function registerLogicalPage({ uri, title, sourcePath, component, accessMode = 'private' }, config) {
  ...
  db.prepare(`
    ...
    VALUES (@uri, @title, @sourcePath, @sourceExt, @sourceRootName, @accessMode, @createdAt, @updatedAt)
...
export function getLogicalPage(uri) {
  initPageStore();
  const normalizedUri = normalizeSlug(uri);
  const record = db.prepare('SELECT * FROM logical_pages WHERE uri = ?').get(normalizedUri);
...
export function recordAccessLog({ pageUri, viewerType = 'auth', shareTokenId = null, requestPath = '', status = 200 }) {
  initPageStore();
  db.prepare(`
    INSERT INTO access_logs (page_uri, viewer_type, share_token_id, request_path, status, created_at)
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(pageUri || null, viewerType, shareTokenId, requestPath, status, nowMs());
}
