...
- **Documentation no longer claims share rows are never deleted.** They survive *expiry*, which is what 0.7.6 changed; `unregister <uri>` still deletes a page's share rows, so links to an unregistered page remain untraceable. SKILL.md and `references/pages-cli.md` now name that hole and point at the open decision rather than implying the ledger is already complete. Found by self-audit and independently by review.

### Known gaps
- **`unregister` deletes the ledger rows for that page.** A share → unregister → `share-info` lookup answers `share_not_found`, not "that was `<uri>`, since deleted". Closing it needs more than keeping the rows: the URI lives only in the page row, so it requires either a URI snapshot on the share row or a page tombstone — a semantic change to what unregister means, deliberately not decided inside this PR.
